...
Implémentation inspirée du guide *Application d’Appel Sécurisée* : jeton éphémère dans le QR, géofence **200 m**, **mode en ligne uniquement** (URL publique HTTPS obligatoire), page professeur, et **option** Gmail API.
...
# Éditez .env : coordonnées du cours (lat/lon), rayon 200 m, secret
...
Ouvrez la doc API sur votre URL publique : `https://votre-domaine/docs`.
...
| `GEOFENCE_RADIUS_METERS` | **200** m par défaut — distance max pour scanner. |
| `TOKEN_TTL_SECONDS` | Durée de validité du jeton (ex. 30). |
| `PUBLIC_URL` / `PUBLIC_BASE_URL` | **Obligatoire** — URL HTTPS publique (Render, ngrok). Pas de `127.0.0.1` ni IP locale. |
...
6. Les élèves peuvent scanner depuis **n'importe quel réseau** s'ils sont à **moins de 200 m** du cours (GPS).
